"use client";
// www.agentui.pro/components/agents/agent-activity
...
function getActiveLabel(type: AgentActivityContentType) {
  if (type === "search") return "Recherche en cours…";
  if (type === "tool") return "Ilèmi utilise ses outils…";
  if (type === "trace") return "Ilèmi travaille…";
  if (type === "mixed") return "Ilèmi travaille…";
  return "Ilèmi réfléchit…";
}
...
function getSummary(
  ...
  if (type === "step" || type === "text") {
    return (
      <>
        Réflexion : <span className="tabular-nums">{formatDuration(duration)}</span>
      </>
    );
  }
  if (type === "search") return "Recherche effectuée";
  if (type === "tool") {
    return `${items.length} ${items.length === 1 ? "outil utilisé" : "outils utilisés"}`;
  }
  if (type === "trace") {
    ...
    return `${tools} ${tools === 1 ? "outil" : "outils"}, ${messages} ${messages === 1 ? "réflexion" : "réflexions"}`;
  }
  return `${items.length} ${items.length === 1 ? "étape terminée" : "étapes terminées"}`;
}
